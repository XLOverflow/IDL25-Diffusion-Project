...
class DDIMScheduler(DDPMScheduler):    
    ...
    def step(
        ...
        alpha_prod_t = self.alphas_cumprod[t]
        alpha_prod_t = alpha_prod_t.to(model_output.device, dtype=model_output.dtype)
        if prev_t >= 0:
            alpha_prod_t_prev = self.alphas_cumprod[prev_t]
        else:
            alpha_prod_t_prev = alpha_prod_t.new_ones(())
        alpha_prod_t_prev = alpha_prod_t_prev.to(model_output.device, dtype=model_output.dtype)
        beta_prod_t = 1.0 - alpha_prod_t 
        ...
        coeff = torch.sqrt(torch.clamp(1.0 - alpha_prod_t_prev - std_dev_t ** 2, min=0.0))
        pred_sample_direction = coeff * pred_epsilon
